...
import {
  fetchSearchConfigurationAndTools,
  createSearchTool,
} from '../src/mcp-server/src/search.js';
...
describe('Search Functionality', () => {
  describe('fetchSearchConfigurationAndOpenApi', () => {
    ...
    test('should fetch configuration successfully', async () => {
      ...
      const config = await fetchSearchConfigurationAndTools('test-subdomain');
      expect(config).toEqual(mockConfig);
      expect(axios.get).toHaveBeenCalledWith(
        'https://leaves.mintlify.com/api/mcp/config/test-subdomain',
        {
    ...
    test('should handle fetch errors', async () => {
      (axios.get as ReturnType<typeof vi.fn>).mockImplementation(() => {
        throw new Error("Error fetching invalid-subdomain's MCP config");
      });

      await expect(fetchSearchConfigurationAndTools('invalid-subdomain')).rejects.toThrow(
