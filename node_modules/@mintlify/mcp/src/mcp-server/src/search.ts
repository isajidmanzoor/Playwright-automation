...
export async function fetchSearchConfigurationAndTools(
  subdomain: string
  ...
  try {
    ...
    throw new Error(
      formatErr(err).replace('Request failed with status code 404', `${subdomain} not found`)
    );
  }
}
...
export async function createSearchTool(server: McpServer) {
  const config = await fetchSearchConfigurationAndTools(SUBDOMAIN);
