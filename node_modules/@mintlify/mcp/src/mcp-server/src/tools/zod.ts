...
import { Blob, Buffer, BlobOptions } from 'node:buffer';
import { z } from 'zod';

type FilePropertyBag = BlobOptions & {
  lastModified?: number;
  type?: string;
};

function panic(error: Error): string {
  throw error;
}
...
const WebFile = class File extends Blob {
  ...
  static [Symbol.hasInstance](object: Record<string | symbol, unknown> | null | undefined) {
    return (
      !!object && object instanceof Blob && /^(File)$/.test(String(object[Symbol.toStringTag]))
    );
  }
};
...
export function dataSchemaArrayToZod(schemas: DataSchemaArray | IncrementalDataSchemaArray) {
  const firstSchema = dataSchemaToZod(schemas[0]);
  if (!schemas[1]) {
    return firstSchema;
  }
  const secondSchema = dataSchemaToZod(schemas[1]);
  const zodSchemas: [z.ZodTypeAny, z.ZodTypeAny, ...z.ZodTypeAny[]] = [firstSchema, secondSchema];
...
function getEnumSchema(enumList: string[] | number[], type: 'string' | 'number' | 'integer') {
...
export function dataSchemaToZod(schema: SchemaInput): z.ZodTypeAny {
  ...
  switch (schema.type) {
    ...
    case 'array':
      let itemSchema;
      let arraySchema: z.ZodArray<z.ZodTypeAny> = z.any().array();
      if (Array.isArray(schema.items)) {
        itemSchema = dataSchemaArrayToZod(schema.items);
        if (schema.items.length > 1) {
          arraySchema = itemSchema as z.ZodArray<z.ZodTypeAny>;
        } else {
          arraySchema = itemSchema.array();
        }
      } else {
        itemSchema = dataSchemaToZod(schema.items as SchemaInput);
        arraySchema = itemSchema.array();
      }
    ...
    case 'object':
      ...
      for (const [key, propSchema] of Object.entries(schema.properties)) {
        const zodPropSchema = Array.isArray(propSchema)
          ? dataSchemaArrayToZod(propSchema as DataSchemaArray)
